		...
		try (Scanner scan = new Scanner(System.in)) {
			...
			while (clientChat.carryOn) {
				...
				if (userMsg.equalsIgnoreCase(MessageType.LOGOUT.toString())) {
					...
					// break to do the disconnect
					break;
// === MODIFICACIÓN: Añadimos los comandos ban y unban ===
				} else if (userMsg.toLowerCase().startsWith("ban ")) {
					// Command: ban <username>
					String[] parts = userMsg.split(" ", 2);
					if (parts.length == 2) {
						String bannedUser = parts[1];
						client.sendMessage(new ChatMessage(clientChat.id, MessageType.BAN, bannedUser));
					} else {
					...
					String[] parts = userMsg.split(" ", 2);
					if (parts.length == 2) {
						String unbannedUser = parts[1];
						client.sendMessage(new ChatMessage(clientChat.id, MessageType.UNBAN, unbannedUser));
					} else {
