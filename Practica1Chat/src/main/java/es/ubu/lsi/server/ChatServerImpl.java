...
// === MODIFICACIÓN: Filtramos los Bloqueos ===
		// Obtenemos quien envía la información (sender)
...
// === MODIFICACIÓN: Omitimos el Mensaje ===
			// Omitimos el envío a clientes que han bloqueado al Remitente
			if (isBanned(ct.getUsername(), senderName)) {
	            continue; // Omite en el envío del Mensaje
	        }
			...
			if (!ct.sendMessage(message)) {
				...
				show("Disconnected Client " + ct.username
						+ " removed from list.");
// === MODIFICACIÓN: Eliminamos del HashMap cuando se desconecta ===
				// Eliminamos del HasMap (userPorts) cuando un cliente se desconecta
				if (ct.username != null) {
...
// === MODIFICACIÓN: Eliminmosr del HashMap cuando se desconecta ===
				// Eliminamos el usuario del HashMAp (userPorts)
				if (ct.username != null) {
					userPorts.remove(ct.username);
					show("Removed " + ct.username + " from userPorts map.");
				}
...
// === MODIFICACIÓN: Métodos auxiliares para ban/unban ===
	...
	private ServerThreadForClient findClientById(int id) {
		for (ServerThreadForClient ct : clients) {
			if (ct.getClientId() == id) { // Inicialmente creado como getId pero por conflicto con Thread, se crea getClientId
				return ct;
			}
		}
		return null;
...
// === CORRECCIÓN: putIfAbsent no está disponible en versiones antiguas de Java ===
...
// === MODIFICACIÓN: Añadimos al HashMap de usuarios y puertos ===
				// Almacenamos username y port en el HashMap (userPorts)
		...
		public void run() {
			...
			while (runningThread) {
				...
				case LOGOUT:
					...
					runningThread = false;
					break;				
// === MODIFICACIÓN: Añadimos los casos para BAN y UNBAN ===
				case BAN:
...
// === MODIFICACIÓN: Añadimos getters necesarios ===
		...
		public int getClientId() {
			return id; // Originariamente creado como getId pero por conflicto con Thread se crea como getClientId
		}
